...
vespid-ai 关注可信智能体执行（trustworthy agent execution）所需要的核心系统层：agent runtime、delegated authorization、sandboxed execution、protected actions、portable context / memory，以及面向 operator 的持续工作流能力。
...
项目仍处于早期，但方向明确：把 agent software 真正落地时最难被产品化清晰表达的几层——runtime、authorization、context portability——做成可检查、可组合、可复用的开源基础设施。
